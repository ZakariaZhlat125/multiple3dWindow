...
if (new URLSearchParams(window.location.search).get("clear")) {
  ...
  function createStars() {
	const sphere = new Float32Array(20000); // Increased the array size to generate more stars
	const sizes = new Float32Array(sphere.length / 3);
	...
	for (let i = 0; i < sizes.length; i++) {
	  sizes[i] = 0.8 + Math.random() * (2.5 - 0.8); // Random size between 0.8 and 2.5
	}
	...
	starsGroup.add(points); // Add stars to the starsGroup
  }
